...
name: aletheia
description: >
  Aletheia is epistemic calibration and ground-truth tracking. It resolves labeled Janus
  claims after the fact — confirming, disconfirming, or superseding them. Transforms the
  Abraxas stack from output-focused to practice-focused, tracking whether epistemic
  confidence claims held up over time. Commands: /aletheia confirm, /aletheia disconfirm,
  /aletheia supersede, /aletheia status, /aletheia calibration, /aletheia queue,
  /aletheia audit. Requires Janus System installed.
---
...
Aletheia is epistemic calibration practice made structural.

The word itself is ancient Greek *aletheia* — literally "un-hiddenness." Heidegger reframed this as truth not as correspondence between claim and fact, but as *disclosure* — the process by which what is hidden becomes visible. Aletheia is truth as practice, not truth as possession.

This skill transforms the Abraxas epistemic stack from a system that *produces* labeled output to a system that *tracks* whether its labels held up. It resolves Sol-mode claims after the fact — confirming them, disconfirming them, or noting that they have been superseded — and maintains a calibration ledger that shows, over time, which confidence labels have proven accurate and which have not.

**The core problem Aletheia solves:** Epistemic labeling systems (like Janus) produce confidence labels in real time. But confidence is only meaningful if it is tested against ground truth — if claims are actually resolved and calibration is tracked. Without this feedback loop, labels become theater: users feel more confident, but the system learns nothing. Aletheia closes the loop. It makes calibration practice structural, not optional.

**The core constraint:** Aletheia operates **only on Sol-mode output** (labels: `[KNOWN]`, `[INFERRED]`, `[UNCERTAIN]`, `[UNKNOWN]`). Symbolic material (`[DREAM]` from Nox) is out of scope — it is not truth-testable. If the user bridges Nox material into Sol output through `/bridge`, the Sol output CAN be resolved through Aletheia. The symbolic source is irrelevant once it has been labeled as Sol-mode epistemic output.

**The practice it enables:** Over time, Aletheia produces a personal calibration record. You can see:
- How accurate your `[KNOWN]` labels are (they should be >95% confirmed)
- How accurate your `[INFERRED]` labels are (they should be ~70-85% confirmed; lower is expected)
- How well-calibrated your `[UNCERTAIN]` claims are (they should show meaningful disconfirmation, not 95% confirmation)
- Whether you are exhibiting confirmation bias (95%+ confirmation across all label types = statistically implausible)

This is not a model evaluation tool. This is a personal epistemic feedback system. The ledger belongs to the user; it records the user's history of engaging with uncertainty and learning whether their confidence was warranted.

---

## Integration Points

Aletheia integrates with four other systems:

**Janus System** (required): Aletheia reads from `~/.janus/ledger.md` and `~/.janus/sessions/{uuid}.md`. It never modifies these files. It writes exclusively to `~/.janus/resolutions.md` (new file, created on first use).

**Honest** (optional): Honest commands produce Sol-mode output with confidence labels. These can be resolved through Aletheia.

**Agon** (optional, when released): Agon produces Convergence Reports with labeled claims. Those claims are natural targets for Aletheia resolution.

**Abraxas Oneironautics** (optional): When `/bridge` produces Sol-mode output from Nox material, that output can be resolved through Aletheia.
...
[... 66 more [INFERRED] claims ...]

[UNCERTAIN] — 96 unresolved

[... sorted by age ...]

[UNKNOWN] — 53 unresolved

These remain open indefinitely by design. Check /aletheia audit to see if any have been invalidated.
```
...
LEDGER FORMAT ISSUES (0)
RESOLUTIONS.MD FORMAT ISSUES (0)
...
**When `/bridge` produces Sol output:**

When Abraxas Oneironautics `/bridge` command produces Sol-mode analysis of Nox material, that Sol output is resolvable through Aletheia. The fact that it came from Nox is irrelevant; what matters is that it carries a Sol label and makes an epistemic claim.
...
4. **Documentation:** This SKILL.md and any reference files emphasize that calibration is a practice, not a scorecard.
...
- Copy session UUIDs manually
- Fill out forms with multiple required fields
- Navigate complex menus
- Wait for full ledger scans
...
When Agon skill is released, it will produce Convergence Reports — labeled claims synthesized from adversarial reasoning. These are natural targets for Aletheia resolution.
...
Later:
User: /aletheia confirm "The Shadow in my work represents unintegrated aspects of power"
Aletheia: ✓ Confirmed. Resolution recorded.
```
...
- `aletheia-architecture.md` — Detailed system design, persistence layer, and query algorithms
- `calibration-practice.md` — Epistemic foundations, what calibration means, how to interpret reports
- `resolution-workflow.md` — Step-by-step workflows for common resolution patterns
...
**v1.0** (2026-03-10)
- Initial release
- Core commands: confirm, disconfirm, supersede, status, calibration, queue, audit
- Integration with Janus System, Honest, and Abraxas Oneironautics
- Calibration ledger with disconfirmation tracking and drift detection
