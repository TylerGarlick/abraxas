...
function getAllKnowledgeEntries(): KnowledgeEntry[] {
  ...
  return files
    .map((f) => {
      try {
        return loadKnowledgeEntry(f.replace(".json", ""));
      } catch {
        return null;
      }
    })
...
// === Simple Embedding (TF-IDF based for now, can be upgraded to neural) ===
// This is a placeholder - in production, use fastembed or similar
...
function getVocabulary(): string[] {
  ...
  return Array.from(vocab).slice(0, 1000); // Limit vocab size
}
...
async function semanticRetrieve(args: {
  ...
  minScore?: number;
  tags?: string[];
}): Promise<{ results: SearchResult[]; query: string; totalSearched: number }> {
  ensureDirectories();
  ...
  for (const fragment of allFragments) {
    ...
    if (score >= minScore) {
      results.push({
        fragment,
        score,
        matchedTerms: vocabulary.filter((_, i) =>
          queryEmbedding[i] > 0 && fragment.embedding![i] > 0
        ),
      });
    }
  }
...
async function synthesizeMemory(args: {
  ...
  summaryParts.push(
    `**Synthesized from ${fragments.length} memory fragment(s):**`
  );
  summaryParts.push("");
...
async function updateKnowledgeBase(args: {
  ...
  // Validate related fragments exist
...
async function main() {
      ...
      tools: [
        {
          ...
          description:
            "Perform semantic search across long-term memory fragments. Returns ranked results by relevance.",
          inputSchema: {
            type: "object",
            properties: {
              query: {
        ...
        },
        {
          ...
          description:
            "Create a synthesized summary from multiple memory fragments. Identifies themes, gaps, and connections.",
          inputSchema: {
            type: "object",
            properties: {
              fragmentIds: {
                type: "array",
                items: { type: "string" },
                description: "List of memory fragment IDs to synthesize",
              },
              query: {
                type: "string",
                description: "Optional query context for the synthesis",
              },
              themes: {
                type: "array",
                items: { type: "string" },
                description: "Optional themes to check for coverage",
              },
            },
        ...
        },
        {
          ...
          description:
            "Inject new verified knowledge into the memory layer. Creates a persistent knowledge entry.",
          inputSchema: {
            type: "object",
            properties: {
              title: {
              ...
              verified: {
                type: "boolean",
                description: "Whether this knowledge is verified (default: false)",
                default: false,
              },
              source: {
                type: "string",
                description: "Source of this knowledge (default: manual)",
                default: "manual",
              },
              tags: {
                type: "array",
              ...
              relatedFragmentIds: {
                type: "array",
                items: { type: "string" },
                description: "Related memory fragment IDs",
              },
            },
    ...
    try {
      ...
      switch (name) {
        case "semantic_retrieve":
          result = await semanticRetrieve(args as any);
          break;
        case "synthesize_memory":
          result = await synthesizeMemory(args as any);
          break;
        case "update_knowledge_base":
          result = await updateKnowledgeBase(args as any);
          break;
        default:
  ...
  console.error("Mnemosyne Memory MCP Server running on stdio");
}
