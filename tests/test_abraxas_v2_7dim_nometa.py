...
def run_batched_test(queries: List[Any], scoring_fn, start_idx=0, end_idx=100, **kwargs) -> Dict[str, Any]:
    """Generic batch processor for any dimension. Respects START_INDEX and END_INDEX."""
    ...
    # SLICE THE QUERIES BASED ON BATCH BOUNDARIES
    relevant_queries = query_list[start_idx:end_idx]
    
    for i in range(0, len(relevant_queries), BATCH_SIZE):
        batch = relevant_queries[i : i + BATCH_SIZE]
        responses = call_ollama_batch(batch)
        
        for idx, resp_text in enumerate(responses):
            q_original = queries[i + idx] if i + idx < len(queries) else "N/A"
            results["total"] += 1
