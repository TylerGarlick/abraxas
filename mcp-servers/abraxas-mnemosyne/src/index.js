...
function extractCrossSkillIds(content) {
    const janusRegex = /jl-\d{8}-[a-z0-9]{6,8}/gi;
    const mnemonRegex = /mb-\d{8}-[a-z0-9]{6,8}/gi;
    const logosRegex = /lg-\d{8}-[a-z0-9]{6,8}/gi;
    const kairosRegex = /kr-\d{8}-[a-z0-9]{6,8}/gi;
    return {
...
export async function executeSessionSave(title, content, metadata = {}) {
    ensureDirectories();
...
export async function executeSessionLoad(sessionId) {
...
export async function executeSessionList(status = 'all', limit = 20, offset = 0) {
    const index = readIndex();
...
export async function executeSessionArchive(sessionId) {
    const index = readIndex();
...
export async function executeSessionExport(sessionId, format = 'markdown') {
...
export async function executeIndexUpdate(operation, sessionId, data = {}) {
    const index = readIndex();
    const now = new Date().toISOString();
    switch (operation) {
        case 'add':
...
const ID_FORMAT_REGEX = /^(jl|mb|lg|kr|mnemo)-\d{8}-[a-z0-9]{6,8}$/i;
export async function executeSessionLinkAdd(sessionId, targetId, linkType = 'manual') {
    const dirs = [ACTIVE_DIR, RECENT_DIR, ARCHIVED_DIR];
    ...
    if (!session.links[linkType].includes(targetId)) {
        session.links[linkType].push(targetId);
        session.modified = new Date().toISOString();
...
export async function executeSessionLinkValidate(sessionId, targetId) {
...
export async function executeSessionLinksList(sessionId) {
    const session = await executeSessionLoad(sessionId);
    if (session.error) {
        return session;
    }
