 ...
 */
function extractUnknowns(request) {
  const unknowns = [];
  const lowerRequest = request.toLowerCase();
  let id = 1;
  
  // Goal
  if (!hasAnswer(lowerRequest, UNKNOWN_CATEGORIES.GOAL.keywords)) {
    unknowns.push({
      id: id++,
      category: 'GOAL',
  ...
  // Audience
  if (!hasAnswer(lowerRequest, UNKNOWN_CATEGORIES.AUDIENCE.keywords)) {
    unknowns.push({
      id: id++,
      category: 'AUDIENCE',
  ...
  // Format
  if (!hasAnswer(lowerRequest, UNKNOWN_CATEGORIES.FORMAT.keywords)) {
    unknowns.push({
      id: id++,
      category: 'FORMAT',
  ...
  // Success criteria
  if (!hasAnswer(lowerRequest, UNKNOWN_CATEGORIES.SUCCESS.keywords)) {
    unknowns.push({
      id: id++,
      category: 'SUCCESS',
  ...
  // Timeline
  if (!hasAnswer(lowerRequest, UNKNOWN_CATEGORIES.TIMELINE.keywords)) {
    unknowns.push({
      id: id++,
      category: 'TIMELINE',
  ...
  // Data
  if (!hasAnswer(lowerRequest, UNKNOWN_CATEGORIES.DATA.keywords)) {
    unknowns.push({
      id: id++,
      category: 'DATA',
...
function answerQuestion(sessionId, questionId, answer, confidence = 'Confident') {
  ...
  // Extract new unknowns from the answer
  const newUnknowns = extractUnknowns(answer);
  const maxId = Math.max(...session.unknowns.map(u => u.id));
  newUnknowns.forEach((nu, i) => {
    nu.id = maxId + i + 1;
    nu.source = 'derived';
    nu.parentQuestion = questionId;
    session.unknowns.push(nu);
  });
